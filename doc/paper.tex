...
Both are weaker than the original, and are less straightforward to use. Nevertheless, this is the path we will take in our development, partly out of necessity (because in some parts of the development, we really need to ``run'' these lemmas to obtain |<=|/|<| proofs), and partly because just taking |le_lt_dec| as an axiom amounts to cheating. \todo{the former is a lie. paste the new perspective as written in mail to Herman, and/or stuff from our paper}
...
% repeat somewhere that an overestimator could just return "true" all the time, but that would make everything reachable, makign the thing useless for unreachability (and hence, safety) determination.
